...
export function validateFile(file: File, documentType?: string): FileValidationResult {
  ...
  // Check file type
  const isImageFile = SUPPORTED_IMAGE_TYPES.includes(file.type as any);
  const isDocumentFile = SUPPORTED_DOCUMENT_TYPES.includes(file.type as any);
...
export async function processFile(
  ...
  if (createPreview && SUPPORTED_IMAGE_TYPES.includes(file.type as any)) {
    try {
  ...
  if (compress && SUPPORTED_IMAGE_TYPES.includes(file.type as any) && file.size > 1024 * 1024) {
    try {
